        ...
        for index, row in data.iterrows():
            ...
            if announcement:
                ...
                *{Role}*
                ...
                Kami hendak menyampaikan info mengenai:
                 • *Subject:* {row.get('Subject', 'N/A')}
                 • *Batas Tanggal Pembayaran:* {row.get('expired_date', 'N/A')}
                 • *Sebesar:* Rp. {row.get('trx_amount', 0):,.0f}
                 • *Pembayaran via nomor virtual account (VA) BNI/Bank:* {row.get('virtual_account', 'N/A')}
                ...
                *Catatan:*
                ...
                Kami hendak menyampaikan info mengenai SPP:
                 • *SPP yang sedang berjalan:* Rp {row.get('bulan_berjalan', 0):,.0f} ({row.get('Ket_1', 'N/A')})
                 • *Denda:* Rp {row.get('Denda', 0):,.0f} ({row.get('Ket_2', 'N/A')})
                 • *SPP bulan-bulan sebelumnya:* Rp {row.get('SPP_30hari', 0):,.0f} ({row.get('Ket_3', 'N/A')})
                 • *Virtual Account (VA)/No. Akun Bank:* {row.get('virtual_account', 'N/A')}
                 • *Keterangan:* {row.get('Ket_4', 'N/A')}
                 • Total tagihan: Rp *{row.get('Total', 0):,.0f}*
                Jika ada pertanyaan atau hendak konfirmasi dapat menghubungi {schooladmin}: https://wa.me/62{sender_number}
                Terima kasih atas kerjasamanya.
                ttd
                *{Role}*
